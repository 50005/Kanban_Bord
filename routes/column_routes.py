...
from schemas.column import ColumnCreate, ColumnUpdate, ColumnModel
...
def create_column():
    ...
    return render_template("column/view.html", column=ColumnModel.from_orm(created_column)), 201
...
def get_column_by_id(column_id: int):
    ...
    return render_template("column/view.html", column = ColumnModel.from_orm(column))
...
def get_edit_column_form(column_id: int):
   ...
   if not column:
     return jsonify({"message": "Column not found"}), 404
   return render_template("column/edit.html", column = ColumnModel.from_orm(column))
...
def update_column(column_id: int):
    ...
    if not updated_column:
        return jsonify({"message": "Column not found"}), 404
    return render_template("column/view.html", column=ColumnModel.from_orm(updated_column))
...
def get_delete_column_form(column_id: int):
   ...
   if not column:
     return jsonify({"message": "Column not found"}), 404
   return render_template("column/delete.html", column=ColumnModel.from_orm(column))
